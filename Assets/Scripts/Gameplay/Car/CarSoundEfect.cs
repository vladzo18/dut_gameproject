...
namespace Gameplay.Car {
    ...
    public class CarSoundEfect : MonoBehaviour {
        ...
        private void Update() {
            if (_carMover.IsMoveing) {
                float egineSpeedPercent = _carMover.CurrentEgineSpeed / _carMover.MaxSpeed;
                _audioSource.volume = Mathf.Lerp(0, _maxEgineSoundVolume, egineSpeedPercent);
            } else {
                _audioSource.volume = 0;
            }
        }
